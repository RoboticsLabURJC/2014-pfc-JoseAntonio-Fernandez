...
\begin{figure}[H]
  \centering
  \includegraphics[scale=0.26]{img/js-jderobot.png}
  \caption{Drone teleoperado con tel\'efono m\'ovil}
  \label{fig:js_jderobot}
\end{figure}
...
\item Desarrollo de una aplicaci\'on GCS Ground Control Station que permita al operador introducir misiones y seguir el cumplimiento de las mismas a trav\'es de el. Permitiendo el acceso a toda la actitud y a las c\'amaras de abordo. Se llamar\'a UAV Commander.
...
\begin{enumerate}
	...
	\begin{enumerate}
		...
		\begin{enumerate}
			\item Pruebas de seguimiento del avi\'on a trav\'es del UAV Commander.	
...
El driver APM Server es quien va a mediar entre las aplicaciones de JdeRobot y el robot a\'ereo con sus sensores y actuadores f\'isicos o simulados a trav\'es de APM Server. De \'esta manera las aplicaciones pueden correr en m\'aquinas distintas, no obligatoriamente abordo, y pueden estar implementadas en distintos lenguajes de programaci\'on. Estas ventajas vienen de utilizar ls divisi\'on habitual en JdeRobot entre componentes drivers y componentes aplicaci\'on.

En nuestro caso \'este driver ira instalado sobre una Raspberry Pi3 con raspbian a la que hemos instalado el framework Jderobot y el driver APM Server, e ir\'a abordo del robot f\'isico conectado por puerto serie.
...
Con estas l\'ineas se inicia el servidor de APM Server y se realiza la conexi\'on con el dispositivo APM o con el simulador.Se puede apreciar c\'omo se realiza la conexi\'on en el m\'etodo \_\_init\_\_  del servidor. Y la creaci\'on de 2 objetos de servidor, una que conectar\'ia con el dispositivo real y la que se realiza con SITL.
...
Los 4 interfaces que van a ser servidos desde APM server ser\'an:
\begin{itemize}
\item Pose3D
\item NavData
\item Extra
...
Desde UAVCommander se pueden programar missiones, enviar \'estas al driver MavLink, verificar el cumplimiento de las mismas y visualizar los sensores a bordo del robot a\'ereo.
    ...
    \includegraphics[width=0.60\textwidth]{img/diseno_uavc.png}}
{
  ...
  \caption{Diseño de UAV Commander}
\end{figure}

La aplicaci\'on UAV Commander se estructura en 3 grandes bloques:
\begin{itemize}
...
\begin{figure}[H]
  ...
  \caption{Diseño de UAV Commander m\'as en profundidad}
  \label{fig:apms_design}
\end{figure}
...
En la aplicaci\'on UAV Commander se exponen 5 interfaces Ice a diferencia de los 4 que expon\'ia el driver APM Server. Adem\'as de los que expone APM Server, a saber:
\begin{itemize}
\item Pose3D
\item NavData
\item Extra
...
Con \'esto se definen los 6 objetos Ice que van a ser enviados y/o recibidos por el APM Server, y con el c\'odigo siguiente se los mapeamos al GUI quien los setear\'a como atributos en su clase principal.
\begin{python}
...
La proyecci\'on en UAV Commander se realiza en ambos sentidos, de imagen a posicion geogr\'afica y de sistema de coordenadas a imagen.
...
Este bloque se encarga de extraer los datos de vuelo y proporcion\'arselos a los componentes del GUI que o necesiten, as\'i como recogerlos del GUI y transformarlos a objetos de interfaz Ice. Este bloque ser\'ia muy similar al bloque de interpretaci\'on del driver APM Server.
...
\begin{figure}[h]
\centering
  \subfloat[Boceto en papel de uno de los diseños de UAV Commander]{
   \label{f:boceto}
    \includegraphics[width=0.486\textwidth]{img/boceto.jpg}}
  \subfloat[Vista general de UAV Commander]{
   \label{f:uavc}
...
\begin{figure}[H]
  ...
  \caption{Pantalla principal de UAV Commander}
  \label{fig:uavc_ppal}
...
\begin{itemize}
\item Un mapa interactivo d\'onde un click crea un punto de misi\'on, dibujando un numero secuencial que muestra el orden la misi\'on. Adem\'as seg\'un van añadi\'endose nuevos puntos UAV Commander va dibujando la ruta que ``deber\'ia'' seguir el robot para el cumplimiento de su missi\'on.
...
se ha conseguido, se ha de justificar, en t\'erminos de qu\'e problem\'as se han
encontrado y qu\'e medidas se han tomado para mitigar esos problem\'as.
