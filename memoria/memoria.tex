...
La aplicación recibe como entradas los interfaces \texttt{Pose3D}, \texttt{NavData}, \texttt{Extra}, \texttt{mission} y \texttt{camera} y recupera, o bien de Google o bien del Instituto Geográfico Nacional de España (IGN), el mapa geo-referenciado de esa zona. Estos datos se presentan por pantalla al usuario a través de las distintas ventanas que coforman el interfaz gráfico. 
...
\begin{itemize}
\item El bloque de sensores. Encargado de recuperar de los servicios ICE, tratar y mostrar toda la información sensorial.
\item El bloque de Localización. Encargado de obtener los mapas geo-referenciados, realizar las proyecciones entre sistemas de coordenadas y mostrar de forma interactiva éstos a través de la interfaz gráfica de usuario.
\item Bloque de control de misiones. El bloque de control de misiones es quien se encarga de facilitar la creación de misiones a través del interfaz gráfico de usuario, realiza las transformaciones necesarias en éstas y las envía a través de los servicios ICE involucrados. 
\end{itemize}
...
La aplicación UAV Commander utiliza 5 interfaces ICE a diferencia de los 4 que usaban en el driver APM Server. Además de los que expone APM Server, a saber: \texttt{Pose3D} y \texttt{NavData} para recibir la posición medida por los sensores a bordo como el GPS o la actitud y \texttt{Extra} y \texttt{Mission} para el envío de misiones con puntos de paso y maniobras de aterrizaje o despegue.
También se utiliza el interfaz \texttt{Camera} para recoger las imágenes y vídeo que se muestran en el interfaz gráfico.


\subsection{Cámara}

\texttt{UAV Commander} utiliza el interfaz \texttt{Camera} para mostrar en el interfaz de usuario las imágenes y vídeos recogidos desde el driver. Para esto primero se crea el servicio ICE y se asigna al interfaz de usuario como podemos observar en la Figura 5.3.
Una vez hecho esto, cuando el usuario hace \textit{click} en el \textit{checkBox} de camera, se abre la ventana de cámara integrada del desarrollo de \texttt{UAV Viewer}. En esta pantalla se recogen los datos de imagen y se asignan al área designado a mostrar las imágenes tal y como se puede ver en la Figura 5.4
...
Para simplificar el proceso de creación y lincado de los servicios ICE hemos optado por utilizar los servidores ya implementadas en JdeRobot, \texttt{CameraClient}, \texttt{NavDataClient}, \texttt{Pose3DClient} y \texttt{Extra} y la clase \texttt{easyiceconfig}. De \'este modo este bloque queda simplificado y su inicializaci\'on queda relegada a unas pocas l\'ineas de c\'odigo como vemos en la Figura 5.5
\begin{figure}[H]
\begin{python}
    ic = EasyIce.initialize(sys.argv)
    camera = CameraClient(ic, "UavViewer.Camera", True)
...
Este bloque es el encargado de la obtención de los mapas gep-referenciados y la interpretación de la posición actual soobre ellos. Este bloque realiza las conversiones de sistemas de referencia necesarios para poder inferir la posición de un punto geofráfico en el planeta a partir de un pixel de la image y viceversa así como las operaciones necesarias para representar ese punto en la imagen.

\subsection{Obtención de Mapas}

Para obtener los mapas geo-referenciados vamos a recurrir al uso de WebMapService (WMS) que es el método más empleado para la obtención de estos mapas. Vamos a utilizar 2 WMS como fuentes, el PNOA del Instituto Geogr\'afico Nacional (IGN) como fuente principal y Google como fuente secundaria.
...
En el c\'odigo mostrado en la Figura 5.6 podemos observar como recuperamos un nuevo mapa del IGN\footnote{Instituto geogr\'afico nacional}, que publica uno de sus numerosos WebMapService (WMS) en 'http://www.ign.es/wms-inspire/pnoa-ma'.
Para obtener un mapa geo-referenciado necesitamos principalmente 4 cosas: la posición, el \textit{bounding box}, el tamaño de la imagen y la proyecci\'on a utilizar. 
Primero obtenemos el \textit{bounding box}. El \textit{bounding box} es la zona geogr\'afica que vamos a recuperar, se calcula a partir de la posici\'on GPS central y, en nuestro caso al mostrar im\'agenes cuadradas, un radio, (el c\'odigo para calcularlo podemos obserbarlo en el método \texttt{getBoundingBox(lat, lon, distance)} de la Figura 5.6).
Una vez obtenido el \textit{bounding box} \'unicamente rellenamos el resto parámetros del objeto WMS y lanzamos la petición que devuelve una imagen en bytes a la que debemos añadir el \textit{disclaimer} que nos indica el IGN para poder utlizar los mapas de forma gratuíta.
Creamos la petición WMS con la URL del WMS PNOA y asignamos los parámetros necesarios para obtener el mapa. al final montamos la imagen geo-referenciada recibida, que en nuestro caso ser\'a un diccionario con la imagen en OpenCV el \textit{bounding box}, y el tamaño de la imagen.
...
\end{python}
\caption{Métodos \texttt{retrieve\_new\_map(lat, lon, radius, width, heigth)} y \texttt{getBoundingBox(lat, lon, distance)} encargado de calcular esquina inferior izquierda y esquina  superior derecha de un cuadrado de ancho \texttt{distance}}
\end{figure}
...
Los datos recibidos del GPS y los solicitados al WMS referencian a una proyección geográfica, la más extendida y utlizamos aquí es la WGS84. Los datos del GPS los recibimos con éste sistema de referencia y solicitamos los mapas en base a él. En nuestro caso trabajamos hasta con 3 sistemas de coordenadas distintos el de las im\'agenes con X[0, MAX\_WIDHT] creciente de izquierda a derecha e Y[0, MAX\_HEIGHT] creciente de arriba a abajo, el sistema de coordenadas geogr\'aficas WGS84 y el cartesiano. Ambas se mueven en \'ordenes de magnitud diferentes y un movimiento de $1 * 10^{-5}$ puede suponer varias decenas de p\'ixeles.

\begin{figure}[H]
\centering
\subfloat[Sistema de coordenadas cartesianas.]{
   \label{f:cartesianas}
    \includegraphics[width=0.31\textwidth]{img/cartesianas.png}}
    \subfloat[Sistema de coordenadas geograficas.]{
   \label{f:geograficas}
    \includegraphics[width=0.31\textwidth]{img/geograficas.jpg}}
    \subfloat[Sistema de coordenadas de una imagen]{
   \label{f:image}
...
Podemos ver la proyecci\'on de posici\'on geogr\'afica a imagen en la Figura 5.7.
\begin{figure}
\begin{python}
...
El mapa se presenta como un elemento más del control de misiones de la aplicación. Para visualizarlo por pantalla se ha creado una capa en PyQt5 con un QLabel que carga la imagen. Para realizar esta tarea de forma ágil y evitar el colápso de los recursos de la máquina donde se ejecuta, trabajamos con opencv, con lo que para mostrarla por pantalla hay que realizar ciertas transformaciones. El código encargado de mostrar por pantalla el mapa se puede ver en la Figura 5.8.
...
Este bloque es el encargado de facilitar la creación y envío de misiones. Ha de permitir crear misiones de forma interactiva además de permitir indentificar y convertir en objetos de interfaz ICE \texttt{mission} las maniobras de despegue y aterrizaje y los puntos de pase.
...
\item \pythoninline{update_position(self)}
	...
	\end{itemize}
\end{itemize}
En la figura  se muestran las implementaciones de los m\'etodos con m\'as peso de \'este bloque.
\begin{figure}[H]
\begin{python}
