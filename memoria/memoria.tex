...
La aplicación UAV Commander utiliza 5 interfaces ICE a diferencia de los 4 que usaban en el driver APM Server. Además de los que expone APM Server, a saber: \textt{Pose3D} y \textt{NavData} para recibir la posición medida por los sensores a bordo como el GPS o la actitud y \textt{Extra} y \textt{Mission} para el envío de misiones con puntos de paso y maniobras de aterrizaje o despegue.
También se utiliza el interfaz \textt{Camera} para recoger las imágenes y vídeo que se muestran en el interfaz gráfico.

\subsection{Cámara}

\texttt{UAV Commander} el interfaz \textt{Camera} para mostrar en el interfaz de usuario las imágenes y vídeos recogidos desde el driver. Para esto primero se crea el servicio ICE y se asigna al interfaz de usuario como podemos observar en la Figura 5.3.
Una vez asignado al Interfaz de usuario, cuando el usuario hace \textit{click} en el \textit{checkBox} de camera, se abre la ventana de cámara integrada del desarrollo de \texttt{UAV Viewer}. En esta pantalla se recogen los datos de imagen y se asignan al área designado a mostrar las imágenes tal y como se puede ver en la Figura 5.4
...
Para simplificar el proceso de creación y lincado de los servicios ICE hemos optado por utilizar las clases implementadas por Aitor Martinez Fernandez\cite{aitor:_mediawiki} y las clases de EasyIce de Victor Arribas Raigadas\cite{varribas:_mediawiki} ambas integradas en JdeRobot. De \'este modo este bloque queda simplificado y su inicializaci\'on queda relegada a unas pocas l\'ineas de c\'odigo como vemos en la Figura 5.12
\begin{figure}[H]
\begin{python}
    ic = EasyIce.initialize(sys.argv)
    camera = CameraClient(ic, "UavViewer.Camera", True)
...
\section{Bloque de Localización}
\label{sec:bloque_localización_uavc}
