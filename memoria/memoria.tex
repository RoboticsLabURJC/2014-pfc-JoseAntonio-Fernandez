...
\Large
ESCUELA SUPERIOR DE INGENIER\'IA INFORM\'ATICA
...
SITL se conecta con JSBSim, un simulador de vuelo de software libre para ejecutar el aspecto f\'isico de la simulaci\'on y con MAVProxy para el env\'io de comandos y seguimiento de misiones. Desde MAVProxy podemos enviar directamente comandos de vuelo al simulador sin necesidad de ninguna herramienta externa, y ha sido muy útil a la hora de articular nuestras pruebas.
...
\cleardoublepage
\chapter{Aplicación AUV Commander}
...
\begin{enumerate}
\item Presentar toda la informaci\'on sensorial que nos ofrezca el driver, la actitud y las c\'amaras de abordo
...
\centering{
   \label{f:diseno_uavc_caja_negra}
    \includegraphics[width=0.8\textwidth]{img/diseno2_uavc.jpg}}
...
\begin{figure}[H]
  ...
  \caption{Esquema de conexión \texttt{UAV Commander}}
  \label{fig:uavc_ppal}
\end{figure}
...
\begin{itemize}
\item El bloque de sensores es el encargado de recuperar desde los interfaces ICE toda la información sensorial a bordo del avión, tratarla y mostrarla
...
\item Bloque de control de misiones es quien se encarga de facilitar la creación de misiones a través del interfaz gráfico de usuario y las envía a través de los interfaces ICE involucrados. 
\end{itemize}

\begin{figure}[H]
\centering{
   \label{f:diseno_uavc_caja_trans}
    \includegraphics[width=0.9\textwidth]{img/diseno_uavc.jpg}}
...
La aplicación contiene cinco hilos asociados a cada uno de los servicios ICE que utiliza y dos hilos más para actualizar en el interfáz gráfico de usuario, la posición en el mapa del avión (\texttt{threadMap}), y los datos recibidos de los sensores (threadGUI).
...
En este bloque se implementan todos los servidores ICE necesarios para acceder a la informaci\'on sensorial y para enviar los comandos de actuaci\'on.
La aplicación \texttt{UAV Commander} utiliza 5 interfaces ICE a diferencia de los cuatro que usa en el driver APM Server. Además de los que expone APM Server (a saber: \texttt{Pose3D} y \texttt{NavData} para recibir la posición medida por los sensores a bordo como el GPS y \texttt{Extra} y \texttt{Mission} para el envío de misiones con puntos de paso y maniobras de aterrizaje o despegue), también utiliza el interfaz \texttt{Camera} para recoger las imágenes y vídeo de la cámara abordo del avión que se muestran en el interfaz gráfico.


\subsection{Cámara}

\texttt{UAV Commander} utiliza el interfaz \texttt{Camera} para obtener las imágenes y vídeos recogidos desde el driver y mostrarlos en su interfaz gráfico. Para esto primero se crea el servicio ICE y se asigna al interfaz de usuario. Una vez hecho esto, cuando el usuario hace \textit{click} en el \textit{checkBox} de camera, se abre la ventana de camera. En esta pantalla se recogen los datos de imagen y se asignan al área dedicado a mostrar las imágenes tal, y como se puede ver en la Figura 5.4
...
Este bloque es el encargado de la obtención de los mapas geo-referenciados y la interpretación de la posición actual sobre ellos. Este bloque realiza las conversiones de sistemas de referencia necesarios para inferir la posición de un punto geográfico en el planeta a partir de un píxel de la imagen y viceversa, así como las operaciones requeridas para representar ese punto geográfico en la imagen.

\subsection{Obtención de Mapas}

Para obtener los mapas geo-referenciados se ha recurrido a WebMapService (WMS) que es el método más empleado para la obtención de estos mapas. Se han utilizado dos WMS como fuentes, el PNOA del Instituto Geogr\'afico Nacional (IGN) como fuente principal y Google como fuente secundaria.

Con el fin de abstraernos del montaje de la URL y env\'io y tratamiento de la respuesta utilizamos la librer\'ia OWSLib\footnote{\url{https://geopython.github.io/OWSLib/}}, en su versión 0.14.0, que nos ayuda con estos tr\'amites. 
...
Primero obtenemos el \textit{bounding box}. El \textit{bounding box} es la zona geogr\'afica que vamos a recuperar, se calcula a partir de la posici\'on GPS central y, en nuestro caso su lado (\texttt{radius}). El c\'odigo para calcularlo podemos observarlo en el método \texttt{getBoundingBox(lat, lon, distance)} de la Figura 5.10.

Una vez obtenido el \textit{bounding box} \'unicamente rellenamos el resto de parámetros del objeto WMS y lanzamos la petición. El WMS devuelve una imagen en bytes a la que debemos añadir el \textit{disclaimer} que nos indica el IGN para poder utlizar los mapas de forma gratuita.
Creamos la petición WMS con la URL del WMS PNOA y asignamos los parámetros necesarios para obtener el mapa. Al final montamos la imagen geo-referenciada, que se implementa como un diccionario con la imagen en OpenCV, el \textit{bounding box}, y el tamaño de la imagen.
...
\end{python}
\caption{Método \texttt{getBoundingBox(lat, lon, distance)} encargado de calcular la esquina inferior izquierda y la esquina superior derecha de un cuadrado de ancho \texttt{distance}}
\end{figure}
...
La proyecci\'on en \texttt{UAV Commander} se realiza en ambos sentidos, de imagen a posición geogr\'afica y de sistema de coordenadas geográficas a imagen.
...
\end{python}
\caption{Método \texttt{posCoords2Image(lonMin, latMin, lonMax, latMax, lat, lon, tamImageX, tamImageY)}, encargado de calcular la proyección de un punto en coordenadas geográficas sobre la matriz de píxeles de la imagen que representa el mapa}
\end{figure}
...
Este bloque es el encargado de facilitar la creación y envío de misiones. Las misiones se definen con una secuencia de puntos de paso (\textit{waypoints}) que se quiere que el drone recorra, incluyendo un punto inicial de despegue y un punto final de aterrizaje. Ha de permitir crear misiones de forma interactiva además de identificar y convertir en objetos de interfaz ICE \texttt{mission} las maniobras de despegue y aterrizaje y los propios puntos de paso.
...
Para dar esta funcionalidad de una forma intuitiva y sencilla se han implementado varios ingredientes:
\begin{itemize}
\item Un mapa interactivo donde un click crea un punto de misi\'on, dibujando un número secuencial que muestra el orden la misi\'on. Adem\'as, seg\'un van añadi\'endose nuevos puntos, la aplicación \texttt{UAV Commander} va dibujando la ruta que ``deber\'ia'' seguir el robot para el cumplimiento de su misi\'on.
Un simple click en el mapa interactivo desencadena:
\begin{enumerate}
\item Inclusi\'on del punto de paso en la lista de puntos de paso.
\item Llamada al m\'etodo de proyecci\'on \pythoninline{posImage2Coords(x, y, sizeX, sizeY, latMin, lonMin, latMax, lonMax)} para obtener a partir del x e y de la imagen la latitud y longitud del punto en el espacio a donde se desea ir.
...
\item En la pr\'oxima iteraci\'on del hilo \texttt{threadMap} a trav\'es de la llamada a \pythoninline{update_position()} se actualizará el mapa con éste cambio.
\item Al actualizars el mapa se llama al método \pythoninline{set_waypoints(self, wayPoints, current=True)} quien pinta el nuevo punto de paso y la ruta a seguir en el mapa. Las l\'ineas de c\'odigo de la figura 5.14 muestran la implementaci\'on del m\'etodo encargado de pintar los puntos de paso y las rutas en el mapa \pythoninline{set_waypoints(self, wayPoints, current=True)}
\end{enumerate}
...
\item Bot\'on ``Send''. Este bot\'on llama al m\'etodo \pythoninline{sendWP(self, send2APM)} (Figura 5.15) quien recoge los objetos de misi\'on, los procesa y los env\'ia al bloque de comunicaci\'on con drivers a trav\'es de los interfaces ICE necesarios, a saber, \texttt{mission} y \texttt{extra} en el caso en el que haya aterrizaje o despegue en la misi\'on.
\end{itemize}
...
\begin{python}
    def sendWP(self, send2APM):
        ...
        for row in range(colCount):
            ...
            i += 1
\end{python}
\caption{Método \texttt{sendWP(self, send2APM)}, encargado de enviar la misión creada por el usuario al driver \texttt{APM Server}}
\end{figure}

\subsection{Control del vuelo}
El control se materializa en la placa estabilizadora a bordo del propio drone, en la aplicación \texttt{UAV Commander} se visualiza su efecto, monitorizando en todo momento la posición 3D del avión y su relación con los puntos de paso de la misión.
...
\item Mapa original con la estela que ha dejado el robot a\'ereo. \pythoninline{self.cvImageShadow}, sus posiciones pasadas.
\item Mapa que va a mostrarse, donde se pinta la posici\'on actual del robot a\'ereo \pythoninline{self.im_to_show}
\end{enumerate}
Este planteamiento por capas permite, por ejemplo, mantener la estela del robot y pintar la posici\'on actual, añadir puntos de paso sin alterar lo ya dibujado, etc. Aunque complica la implementaci\'on de todos los m\'etodos que interact\'uan en este bloque.
Algunos de los m\'etodos más importantes del desarrollo se encuentran aqu\'i y se listan a continuaci\'on:

\begin{itemize}
\item  \pythoninline{update_position(self)}. Este método es al que el hilo \texttt{treadMap} llama en cada iteración. Es el encargado de desencadenar las llamadas suficientes para actualizar visualmente el mapa en el interfaz de usuario.
\item  \pythoninline{self.setPosition(self, x, y, angle)}. Este método es el encargado de realizar las modificaciones sobre el mapa y asignar el nuevo mapa al interfaz de usuario.
\item  \pythoninline{draw_triangle(self, x, y, angle)}. Este método es el encargado de actualizar la posición actual pintando en el mapa un triángulo que muestra la posición y rumbo del avión. El centro del triángulo se sitúa en el píxel que representa la posición geográfica actual, y se gira para que marque el rumbo que sigue el avión.
\item  \pythoninline{refresh_shadow(self,x,y)}. Este método es el encargado de actualizar el mapa de sombras con las posiciones por dónde ha pasado el avión.
\item \pythoninline{set_wayPoints(self.wayPoints)} Este método consulta los puntos de paso pintando éstos sobre el mapa junto con la trayectoria a seguir, si en este momento hay un punto de paso nuevo se añade al mapa con el resto.
\item  \pythoninline{refreshImage(self)}. Este método asigna el nuevo mapa en el interfaz gráfico de usuario.
\end{itemize}

Se ha desarrollado un hilo que actualiza periódicamente el estado del mapa \texttt{threadMap}. Este hilo da la señal al interfaz de usuario de que se tiene que refrescar el mapa, se actualiza a través del método enlazado a esta señal, el método \pythoninline{update_position(self)}.

Para que se pueda ver bien el proceso de una actualización del mapa vamos a describir qué tareas se realizan en cada ciclo de actualización.

En cada iteraci\'on del hilo \texttt{threadMap} se valida si el avión se encuentra en una posición del mapa cercana al borde (menor del 10\% del ancho o alto), si es así se descargará un nuevo mapa de un áera el doble que la primera. Se obtiene la nueva posición y rumbo para que el método \pythoninline{setPosition(self, x, y, angle)} la actualize en el mapa. Se actualiza el mapa de sombras con un nuevo punto correspondiente a la posición actual y se pintan sobre él los puntos de paso en \pythoninline{set_wayPoints(self.wayPoints)}. La siguiente tarea a realizar es actualizar la posición del avión actual sobre el mapa, para ello se dibuja un triángulo de color rojo con el método \pythoninline{draw_triangle(self, x, y, angle)} que representa la posición y el rumbo del avión. Por último se refresca en el interfaz de usuario el mapa que hemos ido modificando con el método \pythoninline{refreshImage(self)}.

A continuación se muestra esta secuencia de llamadas en forma de árbol para que pueda verse mejor que métodos se llaman en qué momento.
\begin{itemize}
...
\subsection{Integración de todo el software}

El objetivo de este experimento es probar que la integración del sistema y el simulador SITL se ha realizado correctamente. Vamos a verificar que se realiza correctamente la conexión con el drone simulado y constatar que el driver recoge correctamente los datos sensoriales del drone. Se verifica también que estos datos se envían correctamente desde el driver a la aplicación y que ésta los representa fielmente. 
Para ello se han seguido los siguientes pasos:
\begin{enumerate}
\item Arranque y configuración de SITL.
\item Se carga una misión de prueba \textit{en el simulador} a través de \texttt{MAVProxy}, donde se le ordena al drone seguir una serie de puntos de paso. Como adelantamos en el capítulo 3 el simulador SITL se integra con MAVProxy y ésto es muy útil, con él podemos cargar misiones desde un fichero, o o comenzar o parar una misión directamente sin necesidad de utilizar nuestra aplicación y driver. 
...
\subsection{Envío de misión con avión en vuelo}

El objetivo de este experimento es probar que el envío de misiones al drone simulado desde nuestra aplicación funciona correctamente y validar su cumplimiento. Para ello se realizaron los siguientes pasos:
\begin{enumerate}
\item Arranque y configuración de SITL.
\item Se carga una misión de prueba a través de \texttt{MAVProxy}, en este caso a diferencia del caso anterior la misión tan solo consta de un despegue. Cuando el autopiloto comprueba que se han alcanzado todas las misiones enviadas entra en modo RTL. En este modo el drone da vueltas trazando una circunferencia de un radio establecido por defecto en el \textit{firmware} de forma indefinida permitíendonos mandarle misiones estando en el aire (Figura 6.1).
\item Se da comienzo a la misión simulada desde \texttt{MAVProxy}.
\item Se arranca el driver \texttt{APM Server} conectando el driver al avión simulado.
...
\begin{figure}[H]
\centering{
   \label{f:experimento_en_vuelo}
    \includegraphics[width=0.9\textwidth]{img/envuelo.png}}
    \caption{Seguimiento del experimento 6.1.2 dentro de la aplicación \texttt{UAV Commander}}
...
\subsection{Envío de misión con despegue y varios puntos de paso}
El objetivo de este experimento es probar que el envío de misiones incluyendo una maniobra de despegue al avión simulado funciona correctamente y validar su cumplimiento. 

Seguimos los mismos pasos que en el experimento anterior salvo por una particularidad, antes de clicar en el primer punto de paso (que será el primer punto al que vuele tras el despegue) pulsaremos el botón de ``Take Off''. De este modo estamos indicando al avión simulado <<Despega hacia el punto de paso X, Y>> donde X e Y serán latitud y longitud de la proyección del píxel de la imagen a coordenadas geográficas. 
...
\begin{figure}[H]
\centering{
   \label{f:experimento_despegue}
    \includegraphics[width=0.8\textwidth]{img/despegue.png}}
    \caption{Seguimiento del experimento 6.1.3 dentro de la aplicación \texttt{UAV Commander}}
  
\end{figure}

\subsection{Envío de misión completa con despegue y aterrizaje}
El objetivo de este experimento es probar que el envío de misiones completas con maniobra de despegue y aterrizaje al avión simulado funciona correctamente y validar su cumplimiento.

Seguiremos los mismos pasos que en el experimento anterior salvo porque antes de clicar en el último punto de paso pulsaremos el botón ``Land''. De este modo estamos indicando al avión simulado <<Aterriza en el punto de paso X, Y>> donde X e Y serán latitud y longitud de la proyección del píxel de la imagen a coordenadas geográficas. 

Durante el experimento se puede comprobar que hay que ajustar muy bien la altitud del avión en los últimos puntos de paso, para que el avión aterrice en lo más cerca posible del último. Como vemos en la Figura 6.3 el avión necesita mucho terreno para aterrizar aún ajustando la altura en sus últimos puntos de paso. La prueba se evidencia en el vídeo \url{https://www.youtube.com/watch?v=4yf_PmFgR5Y}
\begin{figure}[H]
\centering{
   \label{f:experimento completo}
...
El objetivo de este experimento es probar que el autozoom de \texttt{UAV Commander} funciona correctamente. Para ello se han realizado los mismos pasos que en el experimento 6.1.1 estableciendo desde \texttt{MAVProxy} un punto de paso que quede claramente fuera del mapa descargado por defecto por \texttt{UAV Commander}.

Una vez realizada la secuencia arriba descrita se comprueba que \texttt{UAV Commander} descarga un nuevo mapa con una perspectiva más amplia para seguir al avión. Un ejemplo de este funcionamiento lo tenemos en la Figura 6.4. La prueba se evidencia en el vídeo \url{https://www.youtube.com/watch?v=vEcMbJoJ-Qo}
...
\section{Interconexión del driver y aplicación con el avión real}
...
El objetivo de este experimento es verificar que el envío del vídeo capturado desde el ordenador de abordo con \texttt{cameraserver} se recibe correctamente en la aplicación \texttt{UAV Commander}. Para ello se han realizado los siguiente pasos:

\begin{enumerate}
\item Arranque del ordenador abordo.
...
\end{enumerate}
Esta prueba valida la recepción por parte de \texttt{AUV Commander} del vídeo que en este caso se emite desde otro driver distinto de \texttt{APM Server}, tal y como se aprecia en la Figura 6.5. La prueba se evidencia en el vídeo \url{https://youtu.be/U-y8xstytuY}
\begin{figure}[H]
\centering{
   \label{f:pruebavideo}
    \includegraphics[width=0.9\textwidth]{img/ventana_camera.png}}
    \caption{Prueba de recepción de vídeo desde cameraserver a \texttt{UAV Commander}}
  
\end{figure}

\subsection{Recepción de información sensorial y seguimiento del avión a través del \texttt{UAV Commander}}
El objetivo de este experimento es probar que la integración del sistema y el avión real a radiocontrol se ha realizado correctamente. Se valida que se reciben correctamente los datos entregados por el driver y que se presentan fielmente a través del interfaz de usuario. Para ello se realizaron los pasos del experimento anterior salvo que en este caso también moveremos el prototipo en sus 3 ejes de libertad para comprobar que se reciben y se presentan correctamente los datos en la ventana de actitud.

\begin{figure}[H]
\centering{
   \label{f:experimento_sensores_real}
    \includegraphics[width=0.9\textwidth]{img/sensores.png}}
    \caption{Seguimiento del experimento 6.2.3 dentro de la aplicación \texttt{UAV Commander}}
  
\end{figure}

La prueba se evidencia en el vídeo \url{https://youtu.be/HFR7JjD-ZIs}

\subsection{Pruebas de alcance del enlace entre el avión y el ordenador base}
El objetivo de este experimento es probar que se reciben correctamente los datos de actitud del avión desde el ordenador base, según nos vamos alejando. El driver (ejecutando abordo del avión real) y aplicación están conectados a través de wifi y por tanto ésto nos delimitará el alcance al que vamos a volar. Para ello se van a realizar los siguiente pasos:
\begin{enumerate}
\item Arranque del avión a radiocontrol y del ordenador de abordo.
\item Arranque del driver \texttt{APM Server}, este se conecta nada más arrancar a la placa estabilizadora APM a través del puerto de serie.
...
\centering
  ...
  \subfloat[Alcance logrado en la segunda prueba de alcance.]{
   \label{f:alcance2}
    ...
    \caption~{Resultados de las pruebas de alcance wifi}
\end{figure}

\subsection{Experimento de comportamiento ante pérdidas de señal}
El objetivo de este experimento es ver cómo se comporta \texttt{UAV Commander} ante la pérdida de la recepción de \texttt{APM Server}. A tenor de los datos obtenidos en el experimento 6.2.2 este experimento es imprescindible para conocer de antemano qué puede pasar en un vuelo real.
...
En efecto el resultado obtenido corrobora que aplicación y driver retoman con éxito la conexión después de un tiempo sin estar conectados. El experimento demuestra la robustez de ambos desarrollos ante pérdidas de señal, la evidencia puede verse en el vídeo \url{https://youtu.be/HFR7JjD-ZIs}
